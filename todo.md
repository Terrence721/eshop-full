...
| Test coverage | `eShop.ServiceDefaults.UnitTests` (33 tests), `EventBus.UnitTests` (16 tests), `EventBusRabbitMQ.UnitTests` (17 tests), `IntegrationEventLogEF.UnitTests` (11 tests, covers the 5 source files added so far) — 77 tests total — see "Testing strategy" below |
...
`.csproj` added in `0e6e311`. 5 of 7 source files reviewed for correctness and SOLID/DRY/composition-over-inheritance so far:
...
`tests/IntegrationEventLogEF.UnitTests` scaffolded (`MSTest.Sdk`, matching precedent). **All 3 applicable, currently-added source files covered, 11 passing tests.** Tracked as [issue #9](https://github.com/Terrence721/eshop-full/issues/9), sub-issue of #5 — stays open (unlike the other three test-coverage issues) since `IntegrationEventLogEF`'s own source migration is still in progress; the remaining 2 source files get their tests in the same unit of work once added, per the testing-workflow rule.
