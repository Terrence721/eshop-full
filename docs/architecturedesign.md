...
**Current status, stated plainly:** as of this writing, `src/Shared/` (2 linked-source files), `EventBus`, `EventBusRabbitMQ`, `eShop.ServiceDefaults`, `IntegrationEventLogEF`, `Identity.API`, and `Identity.WebApp` are all added and reviewed, with complete test coverage (16, 19, 33, 17, 101, and 118 passing tests respectively, 304 total) — see Section 11. `Identity.API`'s full source tree (`Models`/`Configuration`/`Data`/`Quickstart`/`Services`/`Program.cs`) is done, its Quickstart UI converted directly to a JSON API rather than added as Razor UI first (see Section 9). `Identity.WebApp` (real Vite/React/TypeScript project) is now fully done — all six areas (`Home`/`Account`/`Consent`/`Diagnostics`/`Grants`/`Device`) built, wired into the router, and verified end-to-end with a real browser. A real, significant bug turned up building `Consent`: Duende's own internally-triggered OIDC redirects (login, consent, logout) were landing on `Identity.API`'s raw port instead of the SPA's own origin, because the Vite dev proxy wasn't preserving the original `Host` header — found, fixed with a `proxyReq` hook, and reverified against all three redirect chains plus a full device-flow round trip. `Identity.WebApp` also has its own frontend test suite (Vitest + React Testing Library + jsdom, 118 tests and growing, wired into CI). `Catalog.API` is now in progress — its entire domain-logic layer (`Model`, `Infrastructure` including the DB seeder, `IntegrationEvents`, `Services`) and the real v1/v2 API surface (`Apis/CatalogApi.cs`) are already on disk; only `Program.cs`'s real startup wiring remains. The other 14 projects don't exist on disk yet. Everything below describing the full system is the target this repo is being built toward one file at a time, not a claim that it already runs end-to-end.
...
├── tests/                        ✅ 5 test projects added (186 tests total) — see Section 11.
...
**Testing is no longer a separate end-of-migration phase.** The original plan deferred all of `tests/` to its own migration slot, batched after every service and frontend existed. That's reversed: `tests/eShop.ServiceDefaults.UnitTests` (**all 7 source files, 33 passing tests**), `tests/EventBus.UnitTests` (**all 6 applicable source files, 16 passing tests**), `tests/EventBusRabbitMQ.UnitTests` (**all 7 applicable source files, 19 passing tests**), and `tests/IntegrationEventLogEF.UnitTests` (**all 5 applicable source files, 17 passing tests**) were all added and grown to full coverage — 85 tests total. `tests/Identity.API.UnitTests` (**101 passing tests**) followed the same "every applicable file gets covered" bar, but on a deliberately different schedule: given `Identity.API`'s much larger source tree (129 files vs. single-digit-to-low-teens for every project before it), its test project was deferred until the whole `.csproj`'s source was complete, rather than built file-by-file alongside it — 186 tests total, solution-wide. Going forward, projects at the smaller, single-digit-to-low-teens file-count scale go back to getting their own test project in the same unit of work as their source files; a project `Identity.API`'s size or larger defers testing the same way. `src/Shared/` gets the same treatment despite not being a `.csproj` project itself, since its linked-source files are real logic other projects depend on.

**`Identity.WebApp` got its own frontend test suite for the first time, 2026-09-03** — a genuinely new toolchain, since nothing before it needed one: Vitest + React Testing Library + jsdom, merged into the project's existing `vite.config.ts`. Initially pinned to Vitest 4.1.11 rather than the just-published 5.0.0, which Yarn flagged as quarantined at the time — upgraded to 5.0.0 on 2026-09-08 once it had aged past quarantine, reviewed for real breaking changes (Node 22+/Vite 6.4+ requirements already exceeded; no usage anywhere of the removed `sequential` option or deprecated entry points), and verified clean. 118 tests so far, covering every `api/*.ts` client, the shared `lib/apiFetch.ts`/`lib/useAsync.ts` helpers, the `ScopeSelection`/`ScopeConsentForm` components, and two of the six pages (`Home`, `Login`) — page coverage for the rest is still in progress. Wired into CI the same way the `.NET` side is: a real `Test` step publishing JUnit results via `dorny/test-reporter`. 304 tests total across both toolchains, solution-wide plus `Identity.WebApp`.
