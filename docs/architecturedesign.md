...
**Current status, stated plainly:** as of this writing, `src/Shared/` (2 linked-source files), `EventBus`, `EventBusRabbitMQ`, `eShop.ServiceDefaults`, and `IntegrationEventLogEF` are added and reviewed, with complete test coverage (33, 16, 17, and 17 passing tests respectively, 83 total) — see Section 11. `Identity.API` is in progress: `Models`, `Configuration`, `Data`, and 5 of 6 Quickstart areas (`Home`/`Grants`/`Diagnostics`/`Consent`/`Device`) are done, converted directly to a JSON API rather than added as Razor UI first (see Section 9) — `Account`/`External` and `Services/*`/`Program.cs` remain. The other 15 projects don't exist on disk yet. Everything below describing the full system is the target this repo is being built toward one file at a time, not a claim that it already runs end-to-end.
...
**Testing is no longer a separate end-of-migration phase.** The original plan deferred all of `tests/` to its own migration slot, batched after every service and frontend existed. That's reversed: `tests/eShop.ServiceDefaults.UnitTests` (**all 7 source files, 33 passing tests**), `tests/EventBus.UnitTests` (**all 6 applicable source files, 16 passing tests**), `tests/EventBusRabbitMQ.UnitTests` (**all 6 applicable source files, 17 passing tests**), and `tests/IntegrationEventLogEF.UnitTests` (**all 5 applicable source files, 17 passing tests**) were all added and grown to full coverage — 83 tests total. Going forward every project gets its own test project in the same unit of work as its source files — not deferred. `src/Shared/` gets the same treatment despite not being a `.csproj` project itself, since its linked-source files are real logic other projects depend on.

**Not every source file needs a test file.** `EventBus`'s `IEventBus`/`IEventBusBuilder` are pure interfaces with zero behavior of their own — nothing to assert at runtime that compile-time checking across the rest of the solution doesn't already guarantee. Stated explicitly per file, not silently assumed.
