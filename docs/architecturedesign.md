...
**Current status, stated plainly:** as of this writing, `src/Shared/` (2 linked-source files), `EventBus`, `EventBusRabbitMQ`, and `eShop.ServiceDefaults` are added and reviewed; `IntegrationEventLogEF` is in progress (5 of 7 source files); the other 16 projects don't exist on disk yet. `eShop.ServiceDefaults`, `EventBus`, and `EventBusRabbitMQ` also have complete test coverage now (33, 16, and 17 passing tests respectively), and `IntegrationEventLogEF` has coverage for the 5 source files added so far (11 passing tests) — see Section 11. Everything below describing the full system is the target this repo is being built toward one file at a time, not a claim that it already runs end-to-end.
...
**Testing is no longer a separate end-of-migration phase.** The original plan deferred all of `tests/` to its own migration slot, batched after every service and frontend existed. That's reversed: `tests/eShop.ServiceDefaults.UnitTests` (**all 7 source files, 33 passing tests**), `tests/EventBus.UnitTests` (**all 6 applicable source files, 16 passing tests**), and `tests/EventBusRabbitMQ.UnitTests` (**all 6 applicable source files, 17 passing tests**) were all added and grown to full coverage, and `tests/IntegrationEventLogEF.UnitTests` (**all 3 applicable source files added so far, 11 passing tests**) is keeping pace with that project's still-in-progress source migration rather than waiting for it to finish. Going forward every project gets its own test project in the same unit of work as its source files — not deferred. `src/Shared/` gets the same treatment despite not being a `.csproj` project itself, since its linked-source files are real logic other projects depend on.
