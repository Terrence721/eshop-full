...
**Current status, stated plainly:** as of this writing, `src/Shared/` (2 linked-source files), `EventBus`, `EventBusRabbitMQ`, and `eShop.ServiceDefaults` are added and reviewed; `IntegrationEventLogEF` is in progress (5 of 7 source files); the other 16 projects don't exist on disk yet. `eShop.ServiceDefaults` and `EventBus` also have complete test coverage now (`tests/eShop.ServiceDefaults.UnitTests`, 33 passing tests; `tests/EventBus.UnitTests`, 16 passing tests) — see Section 11. Everything below describing the full system is the target this repo is being built toward one file at a time, not a claim that it already runs end-to-end.
