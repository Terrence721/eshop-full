...
**`Identity.WebApp` got its own frontend test suite for the first time, 2026-09-03** — a genuinely new toolchain, since nothing before it needed one: Vitest 4.1.11 (deliberately not the just-published 5.0.0, which Yarn flagged as quarantined) + React Testing Library + jsdom, merged into the project's existing `vite.config.ts`. 88 tests so far, covering every `api/*.ts` client, the shared `ScopeSelection` component, and two of the six pages (`Home`, `Login`) — page coverage for the rest is still in progress. Wired into CI the same way the `.NET` side is: a real `Test` step publishing JUnit results via `dorny/test-reporter`. 262 tests total across both toolchains, solution-wide plus `Identity.WebApp`.
